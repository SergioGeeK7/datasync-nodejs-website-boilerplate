...
class Query extends Entry {
     ...
     */
    find() {
        console.log("find", this._query)
        if (this.isEmpty(this._query['query']) && Object.keys(this._query).length == 1) {
            ...
            console.log("find else", this._query)
            if (this._query['count']) {
                return this.db.collection("contents")
                    .find({ content_type_uid: this.content_type_uid }).count()
            }
            if (this._query['limit']) {
                return this.db.collection("contents")
                    .find({ content_type_uid: this.content_type_uid }).limit(this._query['limit']).toArray()
            }
            if (this._query['asc'] || this._query['desc']) {
                var asc = this._query['asc']
                var desc = this._query['desc']
                return this.db.collection("contents")
                    .find({ content_type_uid: this.content_type_uid }).sort({ [asc]: 1 ,[desc]: -1}).toArray()
            }
            if (this._query['after_uid']) {
                return this.db.collection("contents")
                    .find({ content_type_uid: this.content_type_uid }).sort({ 'uid': -1 }).toArray()
            }
            if (this._query['before_uid']) {
                return this.db.collection("contents")
                    .find({ content_type_uid: this.content_type_uid }).sort({ 'uid': 1 }).toArray()
            }
            if (this._query['skip']) {
                return this.db.collection("contents")
                    .find({ content_type_uid: this.content_type_uid }).skip(this._query['skip']).toArray()
            }
            ...
            return this.db.collection("contents")
                .find({ content_type_uid: this.content_type_uid, [key]: value }).toArray()
        }
    }
    ...
    */
    findOne(entry_uid) {
        return this.db.collection("contents")
            .findOne({ content_type_uid: this.content_type_uid, uid: entry_uid })
    }
